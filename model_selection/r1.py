...
k = 1
for i in [
    ExtraTreesRegressor(min_samples_leaf=2, n_estimators=102),
    RandomForestRegressor(min_samples_leaf=2, n_estimators=102),
    LGBMRegressor(
        n_estimators=98,
        max_depth=2,
        learning_rate=0.5370528270452327,
        min_split_gain=0.00012531418525182567,
        min_child_weight=0.07922350135025907,
        min_child_samples=11,
        reg_alpha=0.30270847225705616,
        reg_lambda=0.05726625934217654,
    ),
    XGBRegressor(
        n_estimators=78,
        max_depth=2,
        learning_rate=0.5830574529470156,
        min_child_weight=0.07576116552722682,
        reg_alpha=0.3111727161212538,
        reg_lambda=0.501606756809154,
    ),
]:
    # regression model
    ...
    plt.text(
        1, 0.35, f"cross-validation (RMSE): {round(accuracy1.mean(), 2)}", fontsize=10
    )
    plt.text(
        1, 0.20, f"r2_test: {round(metrics.r2_score(y_test, y_pred), 2)}", fontsize=10
    )
    plt.text(
        1,
        0.05,
    ...
    plt.text(
        1,
        -0.40,
        f"r2_train: {round(metrics.r2_score(y_train, y1_pred), 2)}",
        fontsize=10,
    )
    plt.text(
        1,
