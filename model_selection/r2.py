...
for i in [
    ...
    XGBRegressor(
        n_estimators=98,
        max_depth=2,
        learning_rate=0.39162340799554574,
        min_child_weight=0.0852067889182647,
        reg_alpha=0.6866245565279169,
        reg_lambda=0.5217971351407126,
    ),
]:
    # regression model
    ...
    plt.text(
        2, 1.75, f"cross-validation (r2): {round(accuracy.mean(), 2)}", fontsize=10
    )
    plt.text(
        2, 1.65, f"cross-validation (RMSE): {round(accuracy1.mean(), 2)}", fontsize=10
    )
    plt.text(
        2, 1.55, f"r2_test: {round(metrics.r2_score(y_test, y_pred), 2)}", fontsize=10
    )
    plt.text(
        2,
        1.45,
