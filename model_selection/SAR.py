...
x_train, x_test, y_train, y_test = train_test_split(
    x, y.ravel(), test_size=0.2, random_state=10, stratify=y_discretized
)
...
k = 1
for i in [
    ExtraTreesRegressor(min_samples_leaf=2, n_estimators=105),
    RandomForestRegressor(min_samples_leaf=2, n_estimators=127),
    LGBMRegressor(
        n_estimators=100,
        max_depth=2,
        learning_rate=0.26485162360302365,
        min_split_gain=9.598896733776042e-05,
        min_child_weight=0.031405041091885896,
        min_child_samples=14,
        reg_alpha=0.15151696313077256,
        reg_lambda=0.13651480472249508,
    ),
    XGBRegressor(
        n_estimators=119,
        max_depth=2,
        learning_rate=0.4893964080964038,
        min_child_weight=0.030887991255315807,
        reg_alpha=0.03154398899882912,
        reg_lambda=0.14994515092556174,
    ),
]:
    # regression model
    ...
    plt.text(
        2, 1.1, f"cross-validation (RMSE): {round(accuracy1.mean(), 2)}", fontsize=10
    )
    plt.text(
        2, 0.9, f"r2_test: {round(metrics.r2_score(y_test, y_pred), 2)}", fontsize=10
    )
    plt.text(
        2,
        0.7,
        f"MAE_test: {round(metrics.mean_absolute_error(y_test, y_pred), 2)}",
        fontsize=10,
    )
    plt.text(
        2,
        0.5,
        f"MSE_test: {round(metrics.mean_squared_error(y_test, y_pred), 2)}",
        fontsize=10,
    )
    plt.text(
        2,
        0.3,
        f"RMSE_test: {round(np.sqrt(metrics.mean_squared_error(y_test, y_pred)), 2)}",
        fontsize=10,
    )
    plt.text(
        2, 0.1, f"r2_train: {round(metrics.r2_score(y_train, y1_pred), 2)}", fontsize=10
    )
    plt.text(
        2,
        -0.1,
        f"MAE_train: {round(metrics.mean_absolute_error(y_train, y1_pred), 2)}",
        fontsize=10,
    )
    plt.text(
        2,
        -0.3,
        f"MSE_train: {round(metrics.mean_squared_error(y_train, y1_pred), 2)}",
        fontsize=10,
    )
    plt.text(
        2,
        -0.5,
        f"RMSE_train: {round(np.sqrt(metrics.mean_squared_error(y_train, y1_pred)), 2)}",
        fontsize=10,
    )
    k += 1
plt.show()
