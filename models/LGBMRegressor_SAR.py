...
ax.fill_between(x, (x - ci), (x + ci), color="b", alpha=0.3)
...
plt.text(
    1.8,
    0.9,
    f"R2 cross-validation: {round(accuracy.mean(), 2)}",
    fontsize=25,
    family="Arial",
)
plt.text(
    1.8,
    0.65,
    f"RMSE cross-validation: {round(abs(accuracy1.mean()), 2)}",
    fontsize=25,
    family="Arial",
)
plt.text(
    1.8,
    0.4,
    f"R2 test: {round(metrics.r2_score(y_test, y_pred), 2)}",
    fontsize=25,
    family="Arial",
)
plt.text(
    1.8,
    0.15,
    f"RMSE test: {round(np.sqrt(metrics.mean_squared_error(y_test, y_pred)), 2)}",
    fontsize=25,
    family="Arial",
)
plt.text(
    1.8,
    -0.1,
    f"R2 train: {round(metrics.r2_score(y_train, y1_pred), 2)}",
    fontsize=25,
    family="Arial",
)
plt.text(
    1.8,
    -0.35,
    f"RMSE train: {round(np.sqrt(metrics.mean_squared_error(y_train, y1_pred)), 2)}",
    fontsize=25,
    family="Arial",
)
plt.show()
